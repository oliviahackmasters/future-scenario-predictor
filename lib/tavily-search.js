...
export async function fetchTavilyArticles({
  ...
  useDefaultDomains = true
} = {}) {
  if (!process.env.TAVILY_API_KEY || !String(query || "").trim()) return [];

  const baseDomains = useDefaultDomains ? DEFAULT_TAVILY_DOMAINS : [];
  const includeDomains = Array.from(new Set([...baseDomains, ...domains].filter(Boolean)));
